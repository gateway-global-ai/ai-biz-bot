...
| Document | Scope |
|----------|--------|
| **`SKILL_REGISTRY.md`** | Business-facing **capability packs** in `siteConfigs.config.skills` (Bot Builder, menu cards, `siteCapabilities`). **Exception:** `ppp_engagement` is documented there but implemented via **`communication_governance`** + prompt compiler — not `config.skills`. |
