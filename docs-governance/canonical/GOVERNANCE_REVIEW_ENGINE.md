...
## Review sequence
1. Read `SYSTEM_MANIFEST.md` (include `AGENT_SWARM_DEPLOYMENT_RUNBOOK.md` when deploy/provision is in scope; `COMMUNICATION_PLANE_CONTRACT.md` when chat/CGR/ARCH/disclosure is in scope)
...
## Enterprise-scale proposals

When a proposal echoes **external** "full platform" narratives (certification pipelines, PDP/PEP, agent recommendation engines), align language with **`ENTERPRISE_MATURITY_EXTENSIONS.md`** so hypothetical concepts are not mistaken for shipped control-plane contracts.
