...
- **No automated agent** may call `POST /api/v1/admin/knowledge-gap/:siteConfigId/overrides`; only **human superadmin** sessions (`requireAuth` + platform admin). The **Sovereign Sentinel** agent class explains posture and risk using the platform governance artifact and certification state—see [`SOVEREIGN_SENTINEL_POLICY.md`](./SOVEREIGN_SENTINEL_POLICY.md).
