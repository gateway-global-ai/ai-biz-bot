...
const HeroSection: React.FC<Props> = ({ data, isVoiceActive, voiceVolume, onVoiceToggle, onChatClick }) => {
  // Scale factor for the visualizer pulse effect based on volume
            ...
            {!isVoiceActive ? (
                ...
                <div className="flex flex-col items-center gap-8 animate-in fade-in zoom-in duration-300">
                   ...
                   <div className="relative flex items-center justify-center h-40">
                        {/* Outer Glow */}
                        <div 
                            className="absolute w-40 h-40 bg-gradient-to-tr from-blue-500/30 to-purple-500/30 rounded-full blur-3xl transition-all duration-75 ease-linear will-change-transform"
                            style={{ transform: `scale(${scale})` }}
                        />
                         {/* Ripple/Ring Effect */}
                        <div 
                             className="absolute w-32 h-32 border-2 border-white/20 rounded-full transition-all duration-75 ease-linear"
                             style={{ transform: `scale(${scale * 1.1})` }}
                        />
                        <div 
                             className="absolute w-28 h-28 border border-white/40 rounded-full transition-all duration-75 ease-linear"
