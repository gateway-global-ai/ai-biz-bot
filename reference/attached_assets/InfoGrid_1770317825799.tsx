...
const InfoGrid: React.FC<Props> = ({ data, ignoredFields, filteredReviews }) => {
  // Use passed filtered reviews or fallback to all data reviews if not provided
  const displayReviews = filteredReviews || data.reviews;
          ...
          <div className="md:col-span-2 bg-white p-8 rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 flex flex-col">
            ...
            {displayReviews.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-slate-400 text-center p-4">
                    <p className="text-sm">No reviews selected for display.</p>
                </div>
            )}
            
            <div className="mt-6 pt-4 border-t border-slate-50 flex justify-end">
               <a href={data.mapLink} target="_blank" rel="noreferrer" className="text-blue-600 text-sm font-semibold hover:underline">Read all reviews on Google &rarr;</a>
            </div>
          </div>
        )}
