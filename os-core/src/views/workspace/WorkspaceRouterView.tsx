...
export default function WorkspaceRouterView() {
  ...
  useEffect(() => {
    dispatch({
      ...
      payload: {
        ...
        viewId: "workspace-router-view",
        shellMode: "view",
        breadcrumb: ["Home", "Workspace", params.siteId],
      },
    });
    dispatch({
    ...
    const loadQrData = async () => {
      ...
      try {
        const [siteRes, routesRes, chatLogsRes, sessionsRes, summaryRes, outcomeSummaryRes, verificationPolicyRes] = await Promise.all([
          fetch(`/api/site-configs/${params.siteId}`),
          fetch(`/api/qr-routes?page=1&limit=20&siteConfigId=${encodeURIComponent(params.siteId)}`, {
            headers: { ...getAuthHeaders() },
          }),
          fetch(`/api/site-configs/${params.siteId}/chat-logs?limit=20`, {
            headers: { ...getAuthHeaders() },
          }),
          fetch(
            `/api/site-configs/${params.siteId}/frontdesk/sessions?includeResolved=${showResolvedSessions}&limit=300`,
            {
              headers: { ...getAuthHeaders() },
            }
          ),
          fetch(`/api/site-configs/${params.siteId}/conversation-events/summary`, {
            headers: { ...getAuthHeaders() },
          }),
          fetch(`/api/site-configs/${params.siteId}/outcomes/summary?range=today`, {
            headers: { ...getAuthHeaders() },
          }),
          fetch(`/api/site-configs/${params.siteId}/verification-policy`, {
            headers: { ...getAuthHeaders() },
          }),
        ]);
        ...
        if (!cancelled && sessionsRes.ok) {
          ...
          if (nextVersion !== projectionVersion) {
            const mapped = (payload.sessions ?? [])
              .map((session) => mapApiSessionToViewSession(session, params.siteId))
              .sort((a, b) => {
                const priorityDiff = getFrontDeskQueuePriority(a) - getFrontDeskQueuePriority(b);
                if (priorityDiff !== 0) return priorityDiff;
                return a.lastActivityAt < b.lastActivityAt ? 1 : -1;
  ...
  }, [params.siteId, showResolvedSessions]);
  ...
  const handleGenerateWebsiteQr = async () => {
    try {
      setQrMessage("");
      const res = await fetch(`/api/qr/generate/${encodeURIComponent(params.siteId)}`, {
        method: "POST",
      });
      if (!res.ok) throw new Error("Failed to generate QR");
      const refreshedSite = await fetch(`/api/site-configs/${params.siteId}`);
      if (refreshedSite.ok) {
    ...
    try {
      ...
      appendEvent({
        ...
        os_state_snapshot: {
          ...
          breadcrumbs: ["Home", "Workspace", params.siteId],
        },
        payload: {
      ...
      await fetch(`/api/site-configs/${params.siteId}/outcomes/event`, {
        method: "POST",
      ...
      const refreshed = await fetch(`/api/site-configs/${params.siteId}/outcomes/summary?range=today`, {
        headers: { ...getAuthHeaders() },
      });
      if (refreshed.ok) {
  ...
  const handleActivateTelephony = async () => {
    try {
      setQrMessage("");
      const res = await fetch(`/api/site-configs/${params.siteId}/telephony/activate`, {
        method: "POST",
        headers: { ...getAuthHeaders() },
      });
  ...
  const handleVerificationState = async (
    ...
    try {
      await fetch(`/api/site-configs/${params.siteId}/verification/status`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...getAuthHeaders() },
        body: JSON.stringify({
          sessionId: session.sessionId,
                    ...
                    <button
                      ...
                      onClick={() =>
                        void handleDownloadQr(
                          websiteQrImageUrl,
                          `website-qr-${siteConfig?.slug ?? params.siteId}.png`
                        )
                      }
