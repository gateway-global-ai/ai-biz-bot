...
import { Review, ChatMessage, AgentConfig, Agent } from '../types';
...
interface Props {
  ...
  agentConfig: AgentConfig; // Legacy fallback
  
  onToggleField: (field: string) => void;
  onToggleReview: (index: number) => void;
  onSetMinRating: (rating: number) => void;
  onUpdateAgentConfig: (config: AgentConfig) => void; // Legacy fallback
  onClose: () => void;
}

type DevTab = 'data' | 'reviews' | 'integrations' | 'agents';
  ...
  return (
    <div className="w-full h-full flex flex-col bg-slate-50 animate-in slide-in-from-right duration-300">
             ...
             {mode === 'admin' && (
                 <>
                    ...
                    <button 
                        ...
                        className={`pb-3 text-sm font-medium transition-colors whitespace-nowrap ${activeDevTab === 'agents' ? 'text-white border-b-2 border-white' : 'text-white/50 hover:text-white'}`}
                    >
                    Agents Manager
                    </button>
                 </>
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50 p-6 flex flex-col">
          ...
          {mode === 'admin' && activeDevTab === 'agents' && (
             <div className="space-y-6 animate-in fade-in duration-300">
                {/* Agent List */}
                <div className="grid grid-cols-1 gap-4">
                    {agents.map(agent => (
                        <div 
                           key={agent.id} 
                           className={`bg-white rounded-xl border p-4 shadow-sm transition-all ${editingAgentId === agent.id ? 'border-blue-500 ring-1 ring-blue-500' : 'border-slate-200'}`}
                           onClick={() => setEditingAgentId(agent.id)}
                        >
                            <div className="flex justify-between items-start">
                                <div className="flex items-center gap-3">
                                    <div className={`w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold ${agent.type === 'concierge' ? 'bg-blue-100 text-blue-600' : 'bg-purple-100 text-purple-600'}`}>
                                        {agent.name.charAt(0)}
                                    </div>
                                    <div>
                                        <h3 className="font-bold text-slate-900">{agent.name}</h3>
                                        <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">{agent.role}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                                    <label className="relative inline-flex items-center cursor-pointer">
                                      <input 
                                        ...
                                        onChange={() => onUpdateAgent && onUpdateAgent({...agent, enabled: !agent.enabled})}
                                        disabled={agent.isSystem} // Prevent disabling system critical agents if desired, or allow it
                                        className="sr-only peer" 
                                      />
                                      <div className="w-9 h-5 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-blue-600"></div>
                                    </label>
                ...
                {/* Editor Panel for Selected Agent */}
                {editingAgent && onUpdateAgent && (
                    <div className="bg-slate-100 p-6 rounded-2xl border border-slate-200 animate-in slide-in-from-bottom-4">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="font-bold text-slate-800">Editing: {editingAgent.name}</h3>
                            <button onClick={() => setEditingAgentId(null)} className="text-sm text-blue-600 font-bold hover:underline">Done</button>
                        </div>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                          <div>
                             <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">Agent Name</label>
                             <input 
                                type="text"
                                value={editingAgent.name}
                                onChange={(e) => onUpdateAgent({...editingAgent, name: e.target.value})}
                                className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                             />
                          </div>
                          <div>
                             <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">Role Title</label>
                             <input 
                                type="text"
                                value={editingAgent.role}
                                onChange={(e) => onUpdateAgent({...editingAgent, role: e.target.value})}
                                className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                             />
                          </div>
                        </div>
                        ...
                        <div className="mb-6">
                          <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">Voice Identity</label>
                          <select 
                            value={editingAgent.voiceConfig.voiceName}
                            onChange={(e) => onUpdateAgent({...editingAgent, voiceConfig: {...editingAgent.voiceConfig, voiceName: e.target.value as any}})}
                            className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                          >
                            <option value="Zephyr">Zephyr (Calm, Professional)</option>
                            ...
                            <option value="Charon">Charon (Deep, Smooth)</option>
                          </select>
                        </div>

                        <div className="mb-6">
                           <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">System Instructions</label>
                           <textarea 
                              value={editingAgent.basePrompt}
                              onChange={(e) => onUpdateAgent({...editingAgent, basePrompt: e.target.value})}
                              className="w-full h-40 px-4 py-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none font-mono text-sm leading-relaxed resize-none"
                           />
                           <p className="text-xs text-slate-500 mt-2">
                             These instructions define the agent's behavior, knowledge base, and personality traits.
                           </p>
                        </div>
                    </div>
                )}
