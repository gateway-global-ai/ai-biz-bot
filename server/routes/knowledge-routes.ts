...
async function searchPrivateLibrary(projectRoot: string, query: string, limit: number = KB_SEARCH_LIMIT): Promise<Array<{ title: string; library_path: string; excerpt: string }>> {
  ...
  const inRepoItems = await getInRepoLibraryItems(projectRoot) as Array<{ id?: string; title?: string; category?: string; tags?: string[]; library_path?: string }>;
  const allItems = [...indexItems.map((i) => ({ ...i, fromIndex: true })), ...inRepoItems.map((i) => ({ ...i, fromIndex: false }))];
...
router.get('/artifacts/content/:agentAccessKey', async (req: Request, res: Response) => {
  try {
    const agentAccessKey = req.params.agentAccessKey;
