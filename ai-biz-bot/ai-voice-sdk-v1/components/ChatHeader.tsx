...
const MODES: { value: ChatInterfaceMode; label: string; icon: React.ReactNode }[] = [
  ...
  { value: 'ptt', label: 'PTT', icon: <Radio size={16} /> },
  { value: 'realtime', label: 'Realtime', icon: <Activity size={16} /> },
];
      ...
      <div className="px-4 py-4 flex flex-col items-center gap-4">
        <div className="flex items-center gap-3 w-full justify-center">
          ...
          <div className="text-center">
            <h1 className="text-lg font-bold text-gray-100 truncate max-w-[200px]" title={businessName}>
        ...
        <div className="flex items-center justify-center gap-1 p-1 rounded-xl bg-gray-950 border border-gray-800 w-full">
          {MODES.map((m) => (
            <button
              key={m.value}
              type="button"
              ...
              disabled={disabled}
              className={`
                flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all whitespace-nowrap flex-1
                ${mode === m.value
