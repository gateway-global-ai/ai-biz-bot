...
export const useLiveApi = (model: string, voice: string, systemInstruction: string, config: LiveConfig) => {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      // Sanitize voice name: Map legacy "Chirp" voices or full IDs to the short names required by the Live API
      let effectiveVoice = voice;
      if (voice.includes('Chirp') || voice.includes('en-US') || voice.includes('Neural')) {
        if (voice.toLowerCase().includes('puck')) effectiveVoice = 'Puck';
        else if (voice.toLowerCase().includes('charon')) effectiveVoice = 'Charon';
        else if (voice.toLowerCase().includes('kore')) effectiveVoice = 'Kore';
        else if (voice.toLowerCase().includes('fenrir')) effectiveVoice = 'Fenrir';
        else if (voice.toLowerCase().includes('zephyr')) effectiveVoice = 'Zephyr';
        else effectiveVoice = 'Puck'; // Safe default
        
        addLog('info', `Mapped incompatible voice '${voice}' to '${effectiveVoice}'`);
      }

      addLog('info', `Connecting to ${model} with voice ${effectiveVoice}...`);
