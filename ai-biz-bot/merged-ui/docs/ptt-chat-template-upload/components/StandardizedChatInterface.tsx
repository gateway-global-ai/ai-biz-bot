          ...
          <div className="bg-[#111827] border border-slate-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden shrink-0">
             <div className="flex justify-between items-center mb-6">
                <h4 className="flex items-center gap-2 text-[11px] font-black text-slate-400 uppercase tracking-widest">
                   <span className={`w-2 h-2 rounded-full ${isRecording ? 'bg-red-500 animate-pulse' : (isFinalizing ? 'bg-amber-500 animate-pulse' : 'bg-blue-500')}`}></span>
                   {isRecording ? 'Capturing Signal...' : (isFinalizing ? 'Finalizing...' : 'Standby')}
                </h4>
             ...
             <div className="h-20 flex items-center justify-center gap-2">
                {[...Array(12)].map((_, i) => (
                   <div key={i} className="w-2.5 bg-gradient-to-t from-blue-700 to-blue-400 rounded-full transition-all duration-75" style={{ height: `${isRecording ? Math.max(12, voiceVolume * (400 + Math.random() * 100)) : 6}px` }} />
                ))}
             </div>
          </div>
            ...
            <div className={`h-full ${activeView === 'voice' ? '' : 'p-4 space-y-4'}`}>
              ...
              {activeView === 'dashboard' && authStatus === 'authenticated' && businessData && (
                <AdminPanel 
                  ...
                  minRating={0} 
                  agentConfig={botConfig.agentProfile} 
                  onToggleField={() => {}} 
                  onToggleReview={() => {}} 
                  onSetMinRating={() => {}} 
                  onUpdateAgentConfig={(config) => onUpdateBotConfig && onUpdateBotConfig(config)} 
