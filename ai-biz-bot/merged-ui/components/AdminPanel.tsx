...
import { Review, ChatMessage, AgentConfig } from '../types';
...
type DevTab = 'data' | 'reviews' | 'integrations' | 'agent';
    ...
    <div className="fixed inset-0 z-[100] bg-slate-900/50 backdrop-blur-sm flex justify-end animate-in slide-in-from-right duration-300">
      <div className="w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col">
          ...
          <div className="flex gap-4 border-b border-white/10 overflow-x-auto pb-1 scrollbar-hide">
             {mode === 'admin' && (
                 <>
                    ...
                    <button 
                        onClick={() => setActiveDevTab('agent')}
                        className={`pb-3 text-sm font-medium transition-colors whitespace-nowrap ${activeDevTab === 'agent' ? 'text-white border-b-2 border-white' : 'text-white/50 hover:text-white'}`}
                    >
                    Agent Settings
                    </button>
                 </>
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50 p-6 flex flex-col">
          ...
          {mode === 'admin' && activeDevTab === 'agent' && (
             <div className="space-y-8 animate-in fade-in duration-300">
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                   <div className="mb-6">
                      <h3 className="font-semibold text-slate-900 text-lg">Identity & Role</h3>
                      <p className="text-slate-500 text-sm">Define who your AI agent is and how they should behave.</p>
                   </div>
                   ...
                   <div className="mb-6">
                      <label className="block text-sm font-medium text-slate-700 mb-2">DISC Profile / Personality</label>
                      <input 
                         type="text"
                      ...
                      <p className="text-xs text-slate-400 mt-1">
                        Use this to guide the tone of voice. E.g. "Steadiness (Supportive, Patient)" or "Dominance (Direct, Result-oriented)".
                      </p>
                   </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                   <div className="mb-4">
                      <h3 className="font-semibold text-slate-900 text-lg">System Prompt</h3>
                      <p className="text-slate-500 text-sm">The core instructions that govern the AI's behavior and knowledge base.</p>
                   </div>
                   <textarea 
                      value={agentConfig.basePrompt}
                      onChange={(e) => onUpdateAgentConfig({...agentConfig, basePrompt: e.target.value})}
                      className="w-full h-64 px-4 py-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all font-mono text-sm leading-relaxed"
