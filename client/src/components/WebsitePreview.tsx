...
export default function WebsitePreview({ place, onBack }: WebsitePreviewProps) {
  ...
  const shareText = `${place.name} - ${place.formatted_address}${place.rating ? ` | Rated ${place.rating}/5` : ''}`;
  ...
  const shareActions = [
    ...
    },
    {
      ...
      action: () => window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(shareUrl)}`, '_blank', 'width=600,height=400'),
    },
    {
      label: 'WhatsApp',
      icon: (
