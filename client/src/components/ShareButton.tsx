...
interface ShareButtonProps {
  ...
  shareText?: string;
  shareUrl?: string;
  /** Site config UUID — enables share event tracking when provided. */
  siteConfigId?: string;
  /** UUID of the verified platform user sharing this link — appended as ?ref= param. */
...
export default function ShareButton({ 
  ...
  // Build the canonical share URL — append ?ref= when the user is verified
  const baseUrl = customUrl || (typeof window !== 'undefined' ? window.location.href : '');
  const url = referrerUserId && baseUrl
    ? `${baseUrl}${baseUrl.includes('?') ? '&' : '?'}ref=${encodeURIComponent(referrerUserId)}`
    : baseUrl;
  ...
  /** Fire-and-forget share event to the tracking endpoint. */
  const trackShare = useCallback((platform: string) => {
    if (!siteConfigId) return;
    fetch(`/api/site-configs/${siteConfigId}/share`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ platform, referrerUserId }),
    }).catch(() => { /* non-critical — never block the share action */ });
  }, [siteConfigId, referrerUserId]);
  ...
  const shareActions = [
    {
      ...
      action: () => { trackShare('facebook'); window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`, '_blank', 'width=600,height=400'); },
    },
    {
      label: 'X / Twitter',
      ...
      action: () => { trackShare('twitter'); window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(url)}`, '_blank', 'width=600,height=400'); },
    },
    {
      label: 'LinkedIn',
      ...
      action: () => { trackShare('linkedin'); window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}`, '_blank', 'width=600,height=400'); },
    },
    {
      label: 'WhatsApp',
      ...
      action: () => { trackShare('whatsapp'); window.open(`https://wa.me/?text=${encodeURIComponent(text + ' ' + url)}`, '_blank'); },
    },
    ...
    },
    {
      ...
      action: () => { trackShare('email'); window.location.href = `mailto:?subject=${encodeURIComponent(title)}&body=${encodeURIComponent(text + '\n\n' + url)}`; },
    },
    { type: 'divider' as const },
    {
  ...
  async function handleSendSms(e: React.FormEvent) {
    ...
    setSmsSending(true);
    try {
      const res = await fetch('/api/share/send-sms', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          to: smsPhone.trim(),
          shareUrl: url,
          businessName: title.replace('Check out ', ''),
          siteConfigId,
        }),
      });
      if (!res.ok) {
      ...
      setSmsError(err.message || 'Could not send SMS. Please try again.');
    } finally {
      setSmsSending(false);
    }
  }
        ...
        <div id={`${testIdPrefix}-menu`} role="menu" className={dropdownClass} data-testid={`${testIdPrefix}-dropdown`}>
          ...
          {showSmsPanel ? (
            <div className="px-4 py-3">
              <button
                onClick={() => { setShowSmsPanel(false); setSmsError(''); }}
                className={`flex items-center gap-1.5 text-xs mb-3 ${isDark ? 'text-slate-400 hover:text-slate-200' : 'text-slate-500 hover:text-slate-700'} transition-colors`}
              >
              ...
              {smsSent ? (
                <div className="flex items-center gap-2 text-emerald-500 text-sm py-1">
                ...
                <form onSubmit={handleSendSms} className="flex flex-col gap-2">
                  <p className={`text-xs font-medium ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>Send link via SMS</p>
                  <input
                    type="tel"
                    value={smsPhone}
                    ...
                    autoFocus
                    className={`w-full rounded-lg px-3 py-2 text-sm border outline-none focus:ring-2 focus:ring-indigo-500/40 ${
                      isDark
                  ...
                  <button
                    ...
                    className="w-full rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-medium py-2 transition-colors"
                  >
            ...
            shareActions.map((item, i) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    item.action();
                    if (item.label === 'Text Message') return; // SMS opens sub-panel, don't close
                    if (item.label !== 'Copy Link' && item.label !== 'Link Copied') setIsShareOpen(false);
                  }}
                ...
                >
                  ...
                  {item.label === 'Text Message' && (
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className={`w-3 h-3 ml-auto ${iconColor}`}>
