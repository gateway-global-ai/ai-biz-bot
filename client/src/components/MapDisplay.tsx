...
export interface MapDisplayProps {
  /** Google Maps JS API key (required; parent should fetch from /api/config/maps-key). */
  mapsApiKey: string;
...
const MapDisplay: React.FC<MapDisplayProps> = ({ mapsApiKey, selectedDay, allDays, onBookHotel, onSelectDay, isDarkMode = true, activePoiId, onPoiSelect, flyToFlight, hotelFlyAnimation }) => {
  ...
  if (loadError || authError || !mapsApiKey) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center bg-slate-100 dark:bg-slate-900 p-8 text-center animate-in fade-in">
        ...
        <p className="text-slate-500 dark:text-slate-400 max-w-md mb-2 leading-relaxed">
          {authError
            ? "The Google Maps API key is not authorized for this domain. Update API key restrictions or provide a valid key."
            : "Unable to load Google Maps. Check network and API configuration."}
        </p>
        {authError && (
          <p className="text-slate-500 dark:text-slate-400 max-w-md mb-4 text-sm">
            In Google Cloud Console → APIs &amp; Services → Credentials, find the key below. Set Application restriction to &quot;HTTP referrers&quot; and add your domains, e.g. <span className="font-mono text-xs">https://aibizbot-dev.gatewayglobal.ai/*</span>. Ensure &quot;Maps JavaScript API&quot; is enabled for the key.
          </p>
        )}
        <div className="text-xs text-slate-400 dark:text-slate-500 bg-slate-200 dark:bg-slate-800 p-2 rounded font-mono">
          Key in use: {mapsApiKey ? `${mapsApiKey.substring(0, 8)}...` : "Missing"} (from GOOGLE_MAPS_JS_API or /api/config/maps-key)
        </div>
      </div>
    );
  }
