/**
 * Shared phone → OTP → account pick flow for full-page Login and in-canvas platform sign-in.
 */
import { useState } from 'react';
import { useLocation } from 'wouter';
import { useToast } from '@/hooks/use-toast';
import { motion, AnimatePresence } from 'framer-motion';
import { Phone, Shield, Building2, Loader2, ArrowLeft, ChevronRight, X } from 'lucide-react';
import AIOSMark from '@/components/public/AIOSMark';
import { CANVAS_BG_CLASSNAME } from '@/config/brand';

export type Step = 'phone' | 'otp' | 'options';
...
export type UnifiedOtpSurface = 'fullPage' | 'canvas';

export interface UnifiedOtpFormProps {
  surface: UnifiedOtpSurface;
  onClose?: () => void;
  /** After admin token is stored; default: navigate to /platform */
  onAdminActivated?: () => void;
  /** After customer token is stored; default: navigate to /my-account */
  onCustomerActivated?: () => void;
}

export function UnifiedOtpForm({
  surface,
  onClose,
  onAdminActivated,
  onCustomerActivated,
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      let data: { error?: string; phoneLast4?: string } = {};
      try {
        data = await res.json();
      } catch {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      let data: { error?: string; options?: AccountOption[] } = {};
      try {
        data = await res.json();
      } catch {
        throw new Error(res.ok ? 'Invalid response from server' : `Request failed (${res.status})`);
    ...
    <>
      {isCanvas && onClose && (
        <div className="flex items-center justify-between mb-4 shrink-0">
          <span className="text-xs font-semibold uppercase tracking-widest text-slate-500">Sign in</span>
          <button
            type="button"
            onClick={onClose}
        ...
        {step === 'phone' && (
          ...
          >
            ...
            <p className="text-center text-xs text-slate-400">
              New to Gateway?{' '}
              <a href="/buy" className="text-indigo-600 font-semibold hover:underline">
                Get started →
              </a>
            </p>
          </motion.div>
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                      opt.type === 'admin' ? 'bg-indigo-50 text-indigo-600' : 'bg-emerald-50 text-emerald-600'
                    }`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 truncate">
                      {opt.type === 'admin'
                        ? `${(opt.user as { name?: string }).name || 'Admin'} · ${(opt.user as { role?: string }).role}`
                        : opt.businesses && opt.businesses.length > 0
                          ? `${opt.businesses.length} business${opt.businesses.length > 1 ? 'es' : ''}`
                          : String((opt.user as { email?: string }).email || (opt.user as { phone?: string }).phone || '')}
                    </p>
                  </div>
  ...
  if (isCanvas) {
    return (
      <div
        className={`flex flex-col flex-1 min-h-0 w-full max-w-md mx-auto px-4 py-6 overflow-y-auto ${CANVAS_BG_CLASSNAME}`}
      >
        {inner}
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#050a14] flex flex-col items-center justify-center px-4">
      ...
      <p className="mt-6 text-xs text-slate-600">Gateway Global AI · Sovereign AI OS</p>
    </div>
  );
}
